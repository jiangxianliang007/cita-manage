    ...
    if response.status_code == 200:
        ...
        with open(path,"wb") as file:
           for data in response.iter_content(chunk_size=chunk_size):
               file.write(data)
               size += len(data)
               print('\r'+'[Download progress]: %s %.2f%%' % ('>' *int(size*100/ content_size),float(size/content_size *100)),end='')
    end = time.time()
...
def stop_cita():
    ...
    for ip in iplist:
        cmd = ([('cd %s;cd cita_secp256k1_sha3/;'
                 './env.sh ./bin/cita stop test-chain/%d'
                 %( remotedir,cita_node))])
        cita_node = cita_node + 1
        stop_cita_thread = threading.Thread(target=connect_cita_server,args=(ip,username,passwd,cmd))
...
def start_cita():
    ...
    for ip in iplist:
        cmd = ([('cd %s;cd cita_secp256k1_sha3/;'
                 ...
                 %( remotedir,cita_node))])
        cita_node = cita_node + 1
        start_cita_thread = threading.Thread(target=connect_cita_server,args=(ip,username,passwd,cmd))
...
def check_cita_process():
    ...
    for ip in iplist:
        ...
        cita_node = cita_node + 1
        process_cita_thread = threading.Thread(target=connect_cita_server,args=(ip,username,passwd,cmd))
...
def init_cita(nodes, filename):
    ...
    pwd = os.getcwd()
    os.chdir(file)
    os.system(('./env.sh ./scripts/create_cita_config.py create --super_admin "0x4b5ae4567ad5d9fb92bc9afd6a657e6fa13a2523" --nodes %s') % nodes)
    os.chdir(pwd)
...
def put_cita_file(ip, username, passwd, localpath, remotepath):
    ...
    try:
        ...
        print ("Server %s %s ,please check if the user password is correct." % (ip,e))
...
def connect_cita_server(ip, username, passwd, cmd):
    ...
    try:
        ...
        print ('Server %s connection failed, please check if the user password is correct.\n' % (ip))
...
def deploy(iplist ,file):
    ...
    if os.path.exists(filename):
        ...
        response = requests.get("https://api.github.com/repos/cryptape/cita/releases/latest",stream=True)
        ...
        for urldata in cita_down_data:
            ...
            if cita_name in  url_list:
                ...
                path = dir + '/' +cita_name
                print(path)
                print(('Download the latest version of CITA from github...! URL: %s') %url)
                downloader(url,path)
        init_cita(nodes,filename)
        execut()
...
def execut():
    ...
    cita_node = 0
    for ip in iplist:
        put_thread = threading.Thread(target=put_cita_file,args=(ip,username,passwd,localpath,remotepath))
        put_thread.start()
        put_thread.join()

    for ip in iplist:
        cmd = ([('cd %s;docker rm -f $(docker ps -a -q -f name=cita_run);'
        ...
        cita_node = cita_node + 1
        connect_thread = threading.Thread(target=connect_cita_server,args=(ip,username,passwd,cmd))
...
def check_blockNumber():
    ...
    for node in iplist:
        try:
            response = requests.post(url='http://%s:%d' %(node,port) , headers=headers, data=json.dumps(data),timeout=5)
        except Exception as e:
            #print(e)
            print ("The node %s jsonrpc interface connection timed out. Please check if the chain is running and whether the firewall is open." % (node) )
        else:
...
if __name__=='__main__':
    ...
    exit_flag = False
    menu = {1:'deploy_cita',2:'check_blockNumber',3:'stop_cita',4:'start_cita',5:'check_cita_process',6:'quit'}
    ...
    while not exit_flag:
        ...
        if choice.isdigit():
            ...
            if choice in list(menu.keys()):
                ...
                print("The entered code %d does not exist, please re-select !" % choice)
        else:
            print("=======Invalid option, please enter the number in the menu bar list======")
